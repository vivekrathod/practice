...
#include "Win32UtilitiesLib.h"

bool Win32UtilitiesLib::Win32Utilities::VerifyEmbeddedDigitalSignature(String^ filePath)
{
	const std::wstring wideStringFilePath = MarshalString(filePath);
	LPCWSTR pwszSourceFile = wideStringFilePath.c_str();
	LONG lStatus;
	DWORD dwLastError;
	...
	the	end entity certificate is stored in the trusted publisher store, implying that the user trusts content from this
	publisher.

	3) The end entity certificate has sufficient permission to sign	code, as indicated by the presence of a code signing EKU
	or no EKU.	*/
	...
	// This is not applicable if there is no UI because it changes the UI to accommodate running applications instead of 
	...
	case CRYPT_E_SECURITY_SETTINGS:
		...
		wprintf_s(L"CRYPT_E_SECURITY_SETTINGS - The hash representing the subject or the publisher wasn't "
			L"explicitly trusted by the admin and admin policy has disabled user trust. No signature, publisher "
			L"or timestamp errors.\n");
		break;

	default:
		// The UI was disabled in dwUIChoice or the admin policy  has disabled user trust. lStatus contains the 
